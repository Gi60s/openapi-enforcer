...
export const language: II18nMessagesMap = {
  COMPONENT_NAME_INVALID: 'Component {{componentsNamespace}} property names can only contain letters, numbers, dots, dashes, and underscores. The property {{propertyName}} is not valid.',
  COMPONENT_VERSION_MISMATCH: 'The OpenAPIEnforcer supports OpenAPI specification version {{version}}, but the {{componentName}} object does not support this version.',
  ...
  OPERATION_BODY_FORM_CONFLICT: 'Parameters can include a body parameter or one or more formData parameters, but not both. The body parameter and formData parameter are mutually exclusive.',
  OPERATION_BODY_NOT_UNIQUE: 'Only one body parameter is allowed, but multiple were found.',
  OPERATION_CONSUMES_FORM_DATA: 'When specifying formData or file type parameters you should also specify that the operation consumes either "multipart/form-data" or "application/x-www-form-urlencoded".',
  OPERATION_ID_NOT_UNIQUE: 'Each operationId must be unique, but multiple operations have the same operationId: {{operationId}}.',
  PARAMETER_NAMESPACE_CONFLICT: 'Found one or more parameter conflicts. Parameters cannot share the same "name" and "in" values.',
  PARAMETER_PATH_NOT_DEFINED: 'One or more parameters in the path are not defined as path parameters in the Parameters array: {{parameterNames}}.',
  PARAMETER_NOT_IN_PATH: 'A path parameter was defined in the parameters array that is not found in the path: {{parameterName}}.',
  PATH_ENDINGS_INCONSISTENT: 'Some paths end with a slash and some do not. This inconsistency may confuse the users of your API.',
  ...
  REF_CONFLICT: 'The $ref property is not supported when other fields also exist. See issue https://github.com/OAI/OpenAPI-Specification/issues/2635',
  ...
  SPEC_VERSION_NOT_SUPPORTED: 'The OpenAPIEnforcer does not support OpenAPI specification version {{version}}. These versions are supported: {{supportedVersions}}.',
