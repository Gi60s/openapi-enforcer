...
export const language: II18nMessagesMap = {
  ...
  SCHEMA_ALL_ADDITIONAL_PROPERTIES_CONFLICT: 'The additionalProperties have conflicting values.',
  SCHEMA_ALL_DEFAULTS_CONFLICT: 'Multiple conflicting default found.',
  SCHEMA_ALL_DISCRIMINATORS_CONFLICT: 'Unable to merge multiple discriminator values into one.',
  SCHEMA_ALL_DISCRIMINATOR_MAPPINGS_CONFLICT: 'Conflicting discriminator mappings attempt to map different values to same name.',
  SCHEMA_ALL_ENUMS_CONFLICT: 'Enum values across schemas have are mutually exclusive.',
  SCHEMA_ALL_EXAMPLES_CONFLICT: 'Multiple conflicting example found.',
  SCHEMA_ALL_FORMATS_CONFLICT: 'All items must be of the same format. Found: {{formats}}',
  SCHEMA_ALL_NULLABLES_CONFLICT: 'Unable to merge conflicting nullable allowed values.',
  SCHEMA_ALL_PATTERNS_CONFLICT: 'Unable to merge multiple patterns.',
  SCHEMA_ALL_TYPES_CONFLICT: 'All items must be of the same type. Found: {{types}}',
