...
describe('definitions/schema', () => {
    ...
    describe('additionalProperties', () => {
        ...
        it('is valid for objects', () => {
            const [ , err ] = definition(2, Schema, { type: 'object', additionalProperties: { type: 'string' } });
            expect(err).to.be.undefined;
        });

        it('is not valid for non-objects', () => {
            const [ , err ] = definition(2, Schema, { type: 'array', additionalProperties: { type: 'string' } });
            expect(err).to.match(/Property not allowed: additionalProperties/);
        });

        it('can be a boolean', () => {
            const [ , err ] = definition(2, Schema, { type: 'object', additionalProperties: true });
            expect(err).to.be.undefined;
        });
        ...
        it('it cannot be a string', () => {
            const [ , err ] = definition(2, Schema, { type: 'object', additionalProperties: 'hello' });
    ...
    describe('allOf', () => {
        ...
        it('it does not need a type specified', () => {
            const [ , err ] = definition(2, Schema, { allOf: [] });
            expect(err).to.be.undefined;
        });

        it('must be an array', () => {
            const [ , err ] = definition(2, Schema, { type: 'object', allOf: 'hello' });
    ...
    describe('anyOf', () => {
        ...
        it('is not allowed for v2', () => {
            const [ , err ] = definition(2, Schema, { anyOf: [] });
            expect(err).to.match(/Property not allowed: anyOf/);
        });

        it('is allowed for v3', () => {
            const [ , err ] = definition(3, Schema, { anyOf: [] });
            expect(err).to.be.undefined;
        });
    ...
    describe('default', () => {
        ...
        it('allows default', () => {
            const [ , err ] = definition(2, Schema, { type: 'string', default: 'hello' });
            expect(err).to.be.undefined;
        });

        it('deserializes default', () => {
            const [ def, err ] = definition(2, Schema, { type: 'string', format: 'date', default: '2000-01-01' });
            expect(err).to.be.undefined;
            expect(def.default).to.deep.equal(new Date('2000-01-01'));
        });

        it('must must be the same type as type specified', () => {
            const [ , err ] = definition(2, Schema, { type: 'number', default: 'hello' });
    ...
    describe('deprecated', () => {
        ...
        it('is not allowed for v2', () => {
            const [ , err ] = definition(2, Schema, { type: 'string', deprecated: true });
            expect(err).to.match(/Property not allowed: deprecated/);
        });

        it('is allowed for v3', () => {
            const [ , err ] = definition(3, Schema, { type: 'string', deprecated: true });
            expect(err).to.be.undefined;
        });

        it('must be a boolean', () => {
            const [ , err ] = definition(3, Schema, { type: 'string', deprecated: 'hello' });
            expect(err).to.match(/Value must be a boolean/);
        });

        it('defaults to false', () => {
            const [ def ] = definition(3, Schema, { type: 'string' });
            expect(def.deprecated).to.equal(false);
        });

        it('does not have default for v2', () => {
            const [ def ] = definition(2, Schema, { type: 'string' });
    ...
    describe('description', () => {
        ...
        it('can be a string', () => {
            const [ , err ] = definition(2, Schema, { type: 'string', description: 'hello' });
            expect(err).to.be.undefined;
        });

        it('cannot be a number', () => {
            const [ , err ] = definition(2, Schema, { type: 'string', description: 1 });
            expect(err).to.match(/Value must be a string/);
        });
    ...
    describe('enum', () => {
        ...
        describe('dates', () => {
            ...
            it('allows enum value with matching types', () => {
                const [ def, err ] = definition(2, Schema, { type: 'string', format: 'date', enum: ['2001-01-01'] });
                expect(err).to.be.undefined;
            });
        ...
        describe('object', () => {
            ...
            it('allows enum value with matching types', () => {
                const [ def, err ] = definition(2, Schema, {
                    type: 'object',
                    properties: {
                        a: { type: 'string' }
                    },
    ...
    describe('items', () => {
        ...
        it('is valid for arrays', () => {
            const [ def, err ] = definition(2, Schema, { type: 'array', items: { type: 'string' } });
            expect(err).to.be.undefined;
        });

        it('is required for arrays', () => {
            const [ def, err ] = definition(2, Schema, { type: 'array' });
            expect(err).to.match(/Missing required property: items/);
        });

        it('is not valid for non-arrays', () => {
            const [ def, err ] = definition(2, Schema, { type: 'number', items: { type: 'string' } });
            expect(err).to.match(/Property not allowed: items/);
        });

        it('cannot be a non object', () => {
            const [ def, err ] = definition(2, Schema, { type: 'array', items: 'string' });
            expect(err).to.match(/Value must be a plain object/);
        });
    ...
    describe('maximum and minimum', () => {
        ...
        it('allows maximum for string date', () => {
            const [ def, err ] = definition(2, Schema, { type: 'string', format: 'date', maximum: '2000-01-01' });
            expect(err).to.be.undefined;
    ...
    describe('multipleOf', () => {
        ...
        it('allows multipleOf for number', () => {
            const [ def, err ] = definition(2, Schema, { type: 'number', multipleOf: 2 });
            expect(err).to.be.undefined;
        });

        it('does not allow multipleOf for string', () => {
            const [ def, err ] = definition(2, Schema, { type: 'string', multipleOf: 2 });
    ...
    describe('not', () => {
        ...
        it('is not allowed for v2', () => {
            const [ def, err ] = definition(2, Schema, { not: { type: 'string' } });
            expect(err).to.match(/Property not allowed: not/);
        });

        it('is allowed for v3', () => {
            const [ def, err ] = definition(3, Schema, { not: { type: 'string' } });
            expect(err).to.be.undefined;
        });

        it('validates sub schema', () => {
            const [ def, err ] = definition(3, Schema, {
    ...
    describe('oneOf', () => {
        ...
        it('is not allowed for v2', () => {
            const [ def, err ] = definition(2, Schema, { oneOf: [] });
            expect(err).to.match(/Property not allowed: oneOf/);
        });

        it('is allowed for v3', () => {
            const [ def, err ] = definition(3, Schema, { oneOf: [] });
            expect(err).to.be.undefined;
        });
    ...
    describe('pattern', () => {
        ...
        it('will deserialize value to regular expression', () => {
            const [ def, err ] = definition(2, Schema, { type: 'string', pattern: 'hello' });
    ...
    describe('properties', () => {
        ...
        it('is valid for objects', () => {
            const [ def, err ] = definition(2, Schema, { type: 'object', properties: {} });
            expect(err).to.be.undefined;
        });

        it('is not valid for non-objects', () => {
            const [ def, err ] = definition(2, Schema, { type: 'array', properties: {} });
            expect(err).to.match(/Property not allowed: properties/);
        });

        it('is must be an object', () => {
            const [ def, err ] = definition(2, Schema, { type: 'object', properties: 'hello' });
            expect(err).to.match(/Value must be a plain object/);
        });
    ...
    describe('required', () => {
        ...
        it('must be an array', () => {
            const [ def, err ] = definition(2, Schema, { type: 'object', required: 'hello' });
            expect(err).to.match(/Value must be an array/);
        });
