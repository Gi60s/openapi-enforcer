...
const Constructor   = require('../../bin-old/schema');
...
describe.skip('schema.merge', () => {
    ...
    describe('composite', () => {
        ...
        it('cannot do anyOf, oneOf, or not', () => {
            expect(() => schemas.merge(3, [{ anyOf: [] }])).to.throw(/cannot merge the composites/i);
            expect(() => schemas.merge(3, [{ oneOf: [] }])).to.throw(/cannot merge the composites/i);
            expect(() => schemas.merge(3, [{ not: { type: 'boolean'} }])).to.throw(/cannot merge the composites/i);
        })
